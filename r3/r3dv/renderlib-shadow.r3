...
const float SSDO_RADIUS       = 0.8;   // world units
const float SSDO_BIAS         = 0.05;  // evita self-occlusion
...
// ── Noise ────────────────────────────────────────────────────────────────────
// Ruido 2D sin trig, bien distribuido entre [0,1)
float hash21(vec2 p) {
...
mat3 buildTBN(vec3 N) {
    // Metodo de Duff et al. 2017 — sin singularidades
...
// ── Hemiesfera con distribucion coseno (better sampling) ─────────────────────
vec3 cosineSampleHemisphere(float i, float n, float rnd) {
    // Secuencia de Halton base-2 y base-3, perturbada con ruido
    float phi   = (i / n + rnd * 0.1) * 2.0 * PI;
    // Concentrar muestras hacia la normal con raiz (cosine weighting)
...
// Retorna uv en [0,1] y en .z un flag: 0=fuera de pantalla, 1=dentro
vec3 worldToUV(vec3 wp) {
    vec4 clip = ProjView * vec4(wp, 1.0);
    // Descartar si esta detras de la camara
...
// ── SSDO principal ───────────────────────────────────────────────────────────
// Retorna vec4( colorBounce.rgb, occlusion )
vec4 computeSSDO(vec3 worldPos, vec3 N, vec3 albedo) {
    ...
    for (int i = 0; i < SSDO_SAMPLES; ++i) {
        ...
        // Reconstruir posicion real en ese pixel del G-buffer
        vec3  sWorld  = reconstructWorldPos(sUV, sDepth);

        // ── Distancia entre muestra esperada y geometria real ─────────────
        // Si la geometria esta mas cerca que la muestra → hay oclusion
        float occDist = length(sWorld - worldPos);

        // ── Inverse-Z: comparar en depth directamente ────────────────────
        // Proyectamos worldPos al mismo UV para ver su depth esperado
        vec4  selfClip    = ProjView * vec4(sPos, 1.0);
        float selfNDC_Z   = selfClip.z / selfClip.w; // NDC z de la muestra
        // Con inverse-z: depth del buffer > depth de la muestra = mas cerca = ocluye
        float bufferNDC_Z = sDepth * 2.0 - 1.0;

        // Con inverse-z: un depth mayor en el buffer = fragmento MAS CERCANO
        // La muestra queda detrás de la geometría = oclusión
        ...
        if (occluded) {
            ...
            // Termino direccional SSDO: cuanto apunta la dir hacia la luz
            // (diferencia clave SSDO vs SSAO: modula con direccion de luz)
